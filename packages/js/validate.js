...
$.validator.addMethod("maxDate", function(value, element, param) {
	var parval = param.split('-')
	var maxDate = new Date(parval[1]+'/'+parval[0]+'/'+parval[2]);
	var val = value.split('-')
	var inputDate = new Date(val[1]+'/'+val[0]+'/'+val[2]);
...
$('#txtEmpCardNo').keypress(function(e){ 
	if (this.value.length == 0 && e.which == 44 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 45 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 46 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 47 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 48 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 34 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 35 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 39 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 58 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 59 ){
		return false;
	}
	if (this.value.length == 0 && e.which == 95 ){
		return false;
	}
...
/* ENABLE CAPS ALPHA,NUMBER KEY PRESS ONLY */
$(document).on('keypress','.keyCapsAlphabetsNumbersOnly',function(e) {
	var inputValue = e.which;
	if(!(inputValue >= 65 && inputValue <= 90) && !(inputValue >= 48 && inputValue <= 57) && (inputValue != 0 && inputValue != 9 && inputValue != 8)) { 
		e.preventDefault(); 
	}
});
...
// 	$('#frmFirmReg').validate({
// 		rules:{
// 			sample1:{required:true,number:true,maxlength:10,minValue:5999999999},
// 			sample2:{email1:true,maxlength:64},
// 			sample3:{dateFormat1:true,minDynDate:[-1,0,-80],maxDynDate:[0,0,-18]},
// 			sample4:{extension:"pdf|jpg", filesize:[200,2000]},
// 			sample:{numbersOnly:true,startsWith:[6,7,8,9],minValue:6000000000,maxValue:9999999999},
// 			sample:{sameAs:'#txtMobNum',address:true,letterspacenumberonly:true},
	// photo:{required:true, extension: "jpg|jpeg",minfilesize:40, maxfilesize:80},
// 		},
// 		messages:{
// 		}
// 	});
